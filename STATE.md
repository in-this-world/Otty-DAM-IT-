...
## 最後更新
2026-07-18 · by Claude (**Phase 4 完成**——i18n 基礎〔`t()`/`setLang()`/`getLang()`+ zh-TW/en 雙語辭典〕、P4-core〔P4-1 戳人需持木棍、P4-2 結算畫面顯示玩家暱稱、P4-3 多人不生成鷹/熊、P4-4 房主可重開回準備室〕、P4-loot〔P4-5 蘑菇吃了變大疊 4 層、P4-6 挖土改權重 loot table:大便/蘑菇/鑽石/背心/稀有帽/落空〕、P4-drawing〔P4-7 準備室共享塗鴉板,批次傳輸+個人清除〕、**P4-endgame〔P4-8 統計、稱號與結算畫面〕全部併回 main。** 本次(P4-8):新增純模組 `src/core/stats.ts`——`tallyEvent`(吃魚/建壩/大便/鑽石,事件驅動)+ `accumulateSwimTime`(逐 tick 取樣 `floating`,不配對進出水事件)+ `assignTitles`(5 項數值優先序 + 稱號池防重複)。`RoomSimulation.stats()`/`LocalAdapter.getStats()` 掛進既有 per-tick step();`RosterEntry.stats` 轉發給前端、`GameScene` 結算畫面每列附稱號小字。`npm run check` **383 測綠**(357→383)。Phase 4 全部完成,詳見 `Docs/P4-endgame_summary.md`。)
