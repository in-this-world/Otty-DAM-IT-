...
## 最後更新
2026-07-23 · by Claude (**P4 重建 — 基礎批完成、待 boss 複審**。前一版 P4〔i18n+P4-1..8+drawing+endgame〕已合併又 **revert**〔不合標準〕,origin/feat/P4-endgame 尚存但棄用;依 水獺蓋水壩/P4_遊戲改進計畫.md 重新以 TDD 打造。**先修綠底**:`arena.test.ts` 缺 `s.water!` null-guard 讓 main 的 `npm run check` 一直紅〔tsc TS18048〕,已補。**P4-0〔feat/P4-i18n〕**:`src/i18n.ts`〔t/setLang/getLang/toggleLang/onLangChange,node 安全,缺鍵回鍵、缺變數保留 {var}〕+ `src/locale/zh-TW|en.ts`〔扁平、依前綴分組、含 title.* 稱號〕;GameScene/Lobby/MobileControls 全字串走 t();大廳每張卡 EN/中 即時切換〔this.rerender 閉包〕;i18n.test 11 條含**字典完整性**〔鍵集/空值/佔位對齊,前一版所缺〕→ 317 綠。**P4-core〔feat/P4-core,接在 i18n 上〕**:P4-1 沒棍不能戳〔applyPoke +reject 'noStick',空手完全 no-op 含 hazard repel;GameScene 本地 toast t('hint.needStick')〕;P4-2 結局玩家名〔純 end-screen.ts:buildEndScreenRows,暱稱→P1/AI 保底;renderOverlay 畫名牌立繪條〕;P4-3 多人無熊鷹〔hazards.filterHazardsForMode + GameConfig.multiplayer;room-sim start 傳 multiplayer:true〕;P4-4 房主 Restart 回等待室〔RoomSimulation.restart:房主限定、清 state、解除準備、觀戰晉升、房主交接;protocol ClientMessage.Restart;DamRoom 處理+stopLoop;LoopbackTransport 亦支援〕。修 3 支既有測〔poke 助手給木棍、tick poke 給木棍、adapter drain 改 move〕→ 338 綠 + build 綠。**待續**〔複審後〕:P4-5 蘑菇、P4-6 戰利品〔feat/P4-loot〕→ P4-8 結算稱號〔feat/P4-endgame〕→ P4-7 共享畫布〔feat/P4-drawing〕;及 P4-4 網路 client 演出〔host Restart 鈕送訊息+回準備室,需 main.ts lobby↔game 可重入 + 雙瀏覽器 E2E〕、netRosterNames 由大廳填入。)
2026-07-13 · by Claude (**P3 連線 全數重建並完成** — 先前 session 於 Fable 5 額度中斷,子代理成果只存在已清除的 sandbox,GitHub/本機/Docs 皆無 → 確認不可復原後依 transcript 保留的設計重建。**P3-01**:純 `RoomSimulation`(core `reduce`@20Hz + 名冊/房主交接/中途觀戰/斷線 30s AI 接管→移除+`n^0.85` 重算)+ `DamRoom`/`LobbySchema` 薄接線 + `protocol`。**P3-02**:`ColyseusAdapter`(預測=本地 dead-reckoning+快照回正、插值=遠端過去內插)+ `transport`/loopback + colyseus.js 接線;共用 GameAdapter 契約 local/colyseus 皆過 + tick-parity 證明 server==local core。**P3-03**:`LobbyController`+DOM 準備室(join link `#/r/ABCD`、觀戰、連線狀態 UX)。**P3-05**:`PlayerProfile`+`profile-store`(暱稱 localStorage/帽圍巾色/名牌,免登入)。**P3-04**:in-process 多人整合(2 人合作勝利、10 人房+觀戰、斷線續玩到勝)。多人=**opt-in**(需 `#/r/ABCD` 或 `?net` 且設定 `VITE_COLYSEUS_URL`),否則單機路徑完全不變 → `main` 保持可玩+綠。`npm run check` **301 測綠** + `vite build` 綠(bundle ~1.37MB,>500kB 警告=既有 P4 code-split);sandbox E2E smoke 綠含視覺基準(單機 boot/render 未回歸;mechanics 2 條為既有 software-GL flaky,還原改動後同樣紅)。**Server hosting 仍延後**(boss 決定):`src/server/index.ts` 為將來 host 進入點,client 靠 `VITE_COLYSEUS_URL` 連。教訓再確認:掛載資料夾就地覆寫既有檔會截斷(BUG-03)——本次一律 sandbox 編輯 + rsync 回寫,並每完成一塊即 push 到 GitHub `feat/P3-net` 以免再遺失。)
